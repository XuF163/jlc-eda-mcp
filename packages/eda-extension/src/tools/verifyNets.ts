...
import { getDocumentSource } from '../handlers/document';
import { getComponentPins } from '../handlers/schematic';
import { bfsReachable, buildAdjacency, parseWiresFromDocumentSource, pointKey, type WireSeg } from './netlist/docSourceWires';
...
export async function verifyNets(args: z.infer<typeof VerifyNetsSchema>): Promise<unknown> {
	...
	const doc = (await getDocumentSource({ maxChars })) as any;
	...
	const getPins = async (primitiveId: string): Promise<Array<Pin>> => {
		...
		const res = (await getComponentPins({ primitiveId })) as any;
	...
	void timeoutMs; // reserved for future use (kept for API compatibility)
	return {
		ok: Object.values(results).every((r) => r.ok),
		results,
		doc: { truncated: Boolean(doc?.truncated), totalChars: Number(doc?.totalChars ?? sourceText.length) },
	};
}
